...
public class BattleSpecification implements java.io.Serializable {
	...
	private static final long INACTIVITY_TIME = 1000000000;
	private final int numRounds;
	private final double gunCoolingRate;
	private final long inactivityTime;
	private final boolean hideEnemyNames;
	private final int sentryBorderSize;
	private static final int MAX_ROBOT_COUNT = 1000;
	...
	public BattleSpecification(BattlefieldSpecification battlefieldSize) {
		...
		this.battlefieldHeight = battlefieldSize.getHeight();
		this.numRounds = 1;
		this.inactivityTime = 10000000;
		this.gunCoolingRate = 0.1;
		this.sentryBorderSize = 51;
		this.hideEnemyNames = false;
	}
	
	public void addRobotToBattleField(RobotSpecification specification, RobotSetup setup) throws Exception{
		if(robotCount<MAX_ROBOT_COUNT){
			robotsList.add(specification);
			initialSetupsList.add(setup);
			robotCount++;
		}
		else
			throw new Exception("MAX ROBOT LIMIT REACHED!!");
	}
	
	public void setAllBots(){
		robots = new RobotSpecification[robotCount];
		initialSetups = new RobotSetup[robotCount];
		for(int i=0;i<robotCount;i++){
			robots[i] = robotsList.get(i);
			initialSetups[i] = initialSetupsList.get(i);
		}
	}
